...
var ftoVentesComm = L.map("ftoVentesComm", { center: [46.37730064858146, 2.1533203125000004], zoom: 6, zoomControl: false, preferCanvas: false, })
...
lightmatter.addTo(ftoVentesComm);

layerControl.addTo(ftoVentesComm);
...
zoomControl.addTo(ftoVentesComm);

/**
 *  Legend control 
 */

var legendFtoVentesComm = L.control({ position: 'bottomleft' });

// Legend
legendFtoVentesComm.onAdd = function (map) {

    var div = L.DomUtil.create('div', 'info legend'),
        grades = [0, 10, 25, 35, 40, 50, 55, 65, 75];
    labels = [];
    ...
    for (var i = 0; i < grades.length; i++) {
        div.innerHTML +=
            '<i style="background:' + getColorFtoVentesComm(grades[i] + 0.01) + '"></i> ' +
...
legendFtoVentesComm.addTo(ftoVentesComm);
...
var ftoVentesJson = L.geoJson(proprioData, { filter: function (feature, layer) { return !communesToRemove.includes(feature.properties.codgeo); }, style: styleFtoVentesComm, onEachFeature: onEachFeature, smoothFactor: 0 }).addTo(ftoVentesComm);

/**
 *  Info control
 */

var infoFtoVentesComm = L.control();

infoFtoVentesComm.onAdd = function (map) {
...
infoFtoVentesComm.update = function (props) {
    this._div.innerHTML = (props ?
        '<b>' + props.libgeo + ' (' + props.codgeo + ')</b><br />' +
        '----<br />' +
        'Taux de primo-accessions parmi les ventes : ' + props.tx_fto_v + '%<br />' +
        'Nombre de ventes : ' + props.nb_v + '<br />' +
        'dont primo-accession : ' + props.nb_v_fto + ' <br />' +
        'hors primo-accession : ' + props.nb_v_not_f + ' <br />'
        : 'Survolez une commune pour plus d\'informations');
};

infoFtoVentesComm.addTo(ftoVentesComm);
...
// default style
function styleFtoVentesComm(feature) {
    return {
        opacity: 0, // border opacity
        fillColor: getColorFtoVentesComm(feature.properties.tx_fto_ct),
        fillOpacity: 0.9
    };
};
...
function highlightFeature(e) {
    ...
    layer.bringToFront();
    infoFtoVentesComm.update(layer.feature.properties);
}

// back to initial style when not hovered anymore
function resetHighlight(e) {
    ftoVentesJson.resetStyle(e.target);
    infoFtoVentesComm.update();
}

// zoom to city when clicked
function zoomToFeature(e) {
    ftoVentesComm.fitBounds(e.target.getBounds());
}
